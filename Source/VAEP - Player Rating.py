...
SBL = StatsBombLoader(getter = "local", root = "uefa-euro-2020-main-data-statsbomb360")#fill in root directory for StatsBomb Data)
...
df_events = SBL.events(game_id = 3795220) #TO-DO add game ID
df_actions = spadl.statsbomb.convert_to_actions(df_events, home_team_id= 914) #TO-DO add Home-Team ID

df_actions = (
  spadl.add_names(df_actions)
  .merge(SBL.teams(game_id=3795220)) #TO-DO add game ID
  .merge(SBL.players(game_id=3795220)) #TO-DO add game ID
)
...
ratings = VAEP_model.rate(df_games.loc[3795220], df_actions) #TO-DO add game ID in []

#get all players of the game
df_players = SBL.players(game_id = 3795220) #TO-DO add game-ID
